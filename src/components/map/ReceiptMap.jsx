...
export default function ReceiptMap({
  ...
  showDesktopOverlays = true,
  showMobilePanel = true,
  showMaplibreControls = true,
  minHeight = 400,
}) {
  ...
  const updateMapMarkersRef = useRef(() => {});
  const clusterDataRef = useRef([]);
  const locatedReceiptsRef = useRef([]);
  const updateRafRef = useRef(null);
  const originalCreateImageBitmapRef = useRef(null);
  const imageBitmapDisabledRef = useRef(false);
  const maplibreConfiguredRef = useRef(false);
  ...
  }, [locatedReceipts, settings?.baseCurrency, getBaseAmount]);

  const resolveBaseAmount = useCallback((receipt) => {
    if (typeof getBaseAmount === 'function') {
      return getBaseAmount(receipt);
    }
    if (receipt?.id && baseAmountById[receipt.id] !== undefined) {
      return baseAmountById[receipt.id];
    }
    return parseFloat(receipt?.total) || 0;
  }, [getBaseAmount, baseAmountById]);

  const resolveSignedBaseAmount = useCallback((receipt) => {
    const raw = parseFloat(receipt?.total) || 0;
    const base = resolveBaseAmount(receipt);
  ...
  const updateMapMarkers = useCallback(() => {
    ...
    const seen = new Set();
      ...
      if (feature.properties.cluster) {
        ...
        const amount = resolveSignedBaseAmount(receipt);
        ...
        el.title = countHere > 1
          ...
          : `${receipt.merchant} - ${formatCurrency(amount, settings?.baseCurrency || 'EUR')}`;
  ...
  }, [isMapLoaded, onReceiptClick, coordCounts, resolveSignedBaseAmount, settings?.baseCurrency]);
  ...
  useEffect(() => {
    locatedReceiptsRef.current = locatedReceipts;
  }, [locatedReceipts]);
  ...
  useEffect(() => {
    ...
    if (!maplibreConfiguredRef.current) {
      maplibregl.maxParallelImageRequests = 8;
      maplibregl.workerCount = 1;
      maplibreConfiguredRef.current = true;
    }
    ...
    try {
      ...
      if (isFirefox && typeof window !== 'undefined' && typeof window.createImageBitmap === 'function') {
        originalCreateImageBitmapRef.current = window.createImageBitmap;
        window.createImageBitmap = undefined;
        imageBitmapDisabledRef.current = true;
      }
      ...
      mapInstance = new maplibregl.Map({
        ...
        customAttribution: '(c) OpenStreetMap contributors, (c) CARTO',
      ...
      if (imageBitmapDisabledRef.current) {
        window.createImageBitmap = originalCreateImageBitmapRef.current;
        originalCreateImageBitmapRef.current = null;
        imageBitmapDisabledRef.current = false;
      }
    ...
    const handleContextLost = (event) => {
      ...
      if (DEBUG) {
        console.warn('[ReceiptMap] WebGL context lost; reinitializing map.');
      }
      try {
        map.current?.remove();
      } catch (error) {
        console.warn('[ReceiptMap] Error removing map after context loss:', error);
      } finally {
        map.current = null;
        setIsMapLoaded(false);
        setMapReinitKey((key) => key + 1);
      }
    };

    const handleContextRestored = () => {
      if (DEBUG) {
        console.warn('[ReceiptMap] WebGL context restored.');
      }
      setMapReinitKey((key) => key + 1);
    };
    ...
    map.current.on('load', () => {
      ...
      setTimeout(() => {
        if (clusterDataRef.current.length > 0) {
          if (DEBUG) {
            console.log('[ReceiptMap] Loading cluster data:', clusterDataRef.current.length, 'points');
          }
          cluster.current.load(clusterDataRef.current);
          updateMapMarkersRef.current();
          setTimeout(() => {
            if (map.current && locatedReceiptsRef.current.length > 0) {
              if (DEBUG) {
      ...
      if (map.current) {
        try {
          ...
          try {
            ...
            console.warn('[ReceiptMap] Error removing WebGL listeners:', error);
          }
          const events = ['load', 'moveend', 'style.load', 'error', 'data', 'render'];
  ...
  }, [showMaplibreControls, mapReinitKey]);
  ...
  const mapStats = useMemo(() => {
    ...
    const totalAmount = locatedReceipts.reduce((sum, r) => sum + Math.abs(resolveBaseAmount(r)), 0);
  ...
  const formatReceiptInfo = (receipt) => {
    ...
    const amount = resolveSignedBaseAmount(receipt);
    const formattedAmount = formatCurrency(amount, settings?.baseCurrency || 'EUR');
  ...
  return (
    <div className={`relative w-full h-full ${className}`}>
      {/* Map Container */}
      <div ref={mapContainer} className="w-full h-full rounded-xl overflow-hidden shadow-lg" style={{ minHeight }} />
